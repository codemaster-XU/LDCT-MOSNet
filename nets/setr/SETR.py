...
from nets.setr.Transformer import TransformerModel
from nets.setr.PositionalEncoding import (
    FixedPositionalEncoding,
    LearnedPositionalEncoding,
)
from nets.setr.IntmdSequential import IntermediateSequential
...
class SegmentationTransformer(nn.Module):
    ...
    def encode(self, x):
        ...
        if self.conv_patch_representation:
            ...
            """
            # print("分割patches步骤1:",x.shape)
            x = x.view(n, c, -1, self.patch_dim ** 2) #
            # print("分割patches步骤1:", x.shape)
            x = x.permute(0, 2, 3, 1).contiguous()
            # print("分割patches步骤2:", x.shape)
            x = x.view(x.size(0), -1, self.flatten_dim)
            # print("分割patches步骤3:", x.shape)
            x = self.linear_encoding(x)
            # print("分割patches步骤4:", x.shape)
        x = self.position_encoding(x) # position_encoding: torch.Size([1, 900, 768])
...
class SETR_Naive(SegmentationTransformer):
    ...
    def decode(self, x, intmd_x, intmd_layers=None):
        ...
        # print("conv2:",x.shape)
        x = self.upsample(x)
        # print("decoder_upsample:",x.shape)
        return x
